...
console.log(currentuser);
  ...
  return (
      ...
      <div   className="Tasks  w-full h-[300px] p-4 mt-10 rounded-lg border border-border bg-surface overflow-scroll ">
        {
           currentuser.tasks.map((task,id)=>{
              
                 return(
                          
              <div key={id} className="Task  bg-navbar rounded-md">
              <div className="flex flex-row  justify-between items-center px-4 py-2">
                <span className="bg-primary px-2 py-1">High</span>
                <h2 className="text-base">  {task.date} </h2>
              </div>
    
              <div className="flex flex-col items-start justify-center p-4  ">
                <h4 className="text-xl text-text-primary ">  {task.title}</h4>
